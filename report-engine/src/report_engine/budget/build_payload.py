...
def extract_prompts_from_template(template_path: str) -> list:
    """从模板中提取 PROMPT 注释。"""
    # 复用 report_engine.prompt_parser 的逻辑
    try:
        from report_engine.prompt_parser import extract_prompts
        return extract_prompts(template_path)
    except ImportError:
        pass
...
def auto_generate_from_template(
    ...
    # 解析模板完整结构
    try:
        from report_engine.prompt_parser import extract_prompts, extract_template_structure
    ...
    from report_engine.budget.llm_client import generate_section_content
    from report_engine.budget.markdown_to_blocks import markdown_to_blocks
