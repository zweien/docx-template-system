        ...
        <div className={`flex items-center ${collapsed ? "justify-center px-0" : "gap-2.5 px-4"} pt-5 pb-4`}>
          ...
          {!collapsed && (
            <div>
              ...
              <p className="text-text-quaternary leading-tight mt-0.5 font-mono" style={{ fontSize: "0.667em" }}>v0.8.6</p>
            </div>
          )}
        </div>
...
function ChangelogContent() {
  const versions = [
    { ver: "0.8.6", date: "2026-05-07", changes: [
      { type: "fix", text: "修复 Windows 打包版示例文件无法下载的问题" },
    ]},
    { ver: "0.8.5", date: "2026-05-07", changes: [
      { type: "feat", text: "帮助说明：各页面添加 ? 图标帮助气泡，解释配置项含义" },
      { type: "feat", text: "文件拖拽：模板选择、Excel 导入、模板管理、数据校验、合并表格支持拖入文件" },
      { type: "feat", text: "模板管理：多选、全选、批量删除" },
      { type: "fix", text: "修复帮助气泡靠右时溢出屏幕的问题" },
      { type: "fix", text: "修复拖拽文件重复导入的问题" },
      { type: "feat", text: "更新示例文件：匹配模板中文占位符，含示例图片" },
    ]},
