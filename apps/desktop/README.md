...
- 修复 Windows 打包版示例 Excel/JSON 文件无法下载的问题（fetch URL 硬编码 localhost）

### v0.8.5

- 帮助说明：各页面添加 ? 图标帮助气泡，解释配置项含义
- 文件拖拽：模板选择、Excel 导入、模板管理、数据校验、合并表格支持拖入文件
- 模板管理：多选、全选、批量删除
- 修复帮助气泡靠右时溢出屏幕、拖拽文件重复导入的问题
- 更新示例文件：匹配模板中文占位符，含示例图片
...
- 表格样式：通过模板段落样式（表格表头/表格正文）控制表格字体
- 全局配置：图片宽度、科目/条目标题级别统一设置
- 模板管理页面：下载示例模板按钮
- 自定义样式名统一为中文

### v0.8.2

- 科目总体描述：从 Excel 汇总页读取描述插入到 heading 下（table/cell_map 均支持）
- 修复 Word 拆分 run 导致占位符无法识别
- 修复 OMML 命名空间注册产生 xmlns:ns0 错误
