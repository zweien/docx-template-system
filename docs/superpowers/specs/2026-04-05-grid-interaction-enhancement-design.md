...
删除 `renderCell` 中的 `max-w-[200px]`，改为 `overflow: hidden; text-overflow: ellipsis` 处理溢出。
...
实现方式：在 `handleColumnDragEnd` 中检查 `source` 和 `target` 是否都在同一区域（冻结/非冻结），跨区域时忽略。
...
列拖拽的 `DragDropProvider` 包裹 `<thead>`，行拖拽用独立的 `DragDropProvider` 包裹 `<tbody>`。不能用同一个，因为列和行的拖拽语义不同。
...
| `Tab` | 右移，行末跳下一行行首 | 无编辑状态 |
| `Shift+Tab` | 左移，行首跳上一行行末 | 无编辑状态 |
...
| `src/hooks/use-inline-edit.ts` | 小改 | 支持空值提交（Delete 键清空） |
